...
FATO_INTERVENCAO_DOTACAO:
  INNER JOIN DIM_DATA_CALENDARIO ON SK_DATA_CALENDARIO = DIM_DATA_CALENDARIO.SK_DATA_CALENDARIO
  INNER JOIN DIM_UNIDADE_GESTORA ON SK_UNIDADE_GESTORA = DIM_UNIDADE_GESTORA.SK_UNIDADE_GESTORA
  INNER JOIN DIM_FONTE_RECURSO ON SK_FONTE_RECURSO = DIM_FONTE_RECURSO.SK_FONTE_RECURSO
  INNER JOIN DIM_NATUREZA_DESPESA ON SK_NATUREZA_DESPESA = DIM_NATUREZA_DESPESA.SK_NATUREZA_DESPESA
  INNER JOIN DIM_SUBACAO ON SK_SUBACAO = DIM_SUBACAO.SK_SUBACAO
  INNER JOIN DIM_INSTITUCIONAL ON SK_INSTITUCIONAL = DIM_INSTITUCIONAL.SK_INSTITUCIONAL
  INNER JOIN DIM_FORNECEDOR ON SK_FORNECEDOR = DIM_FORNECEDOR.SK_FORNECEDOR
  INNER JOIN DIM_EMENDA_PARLAMENTAR ON SK_EMENDA_PARLAMENTAR = DIM_EMENDA_PARLAMENTAR.SK_EMENDA_PARLAMENTAR
  INNER JOIN DIM_GRUPO_PROG_FINANCEIRA ON SK_GRUPO_PROG_FINANCEIRA = DIM_GRUPO_PROG_FINANCEIRA.SK_GRUPO_PROG_FINANCEIRA

FATO_EXECUCAO_RECEITA:
  INNER JOIN DIM_DATA_CALENDARIO ON SK_DATA_CALENDARIO = DIM_DATA_CALENDARIO.SK_DATA_CALENDARIO
  INNER JOIN DIM_UNIDADE_GESTORA ON SK_UNIDADE_GESTORA = DIM_UNIDADE_GESTORA.SK_UNIDADE_GESTORA
  INNER JOIN DIM_FONTE_RECURSO ON SK_FONTE_RECURSO = DIM_FONTE_RECURSO.SK_FONTE_RECURSO
  INNER JOIN DIM_NATUREZA_RECEITA ON SK_NATUREZA_RECEITA = DIM_NATUREZA_RECEITA.SK_NATUREZA_RECEITA
